...
it would find two matches and return ["a", "a"] because
the a characters are not in a row - there is a b between them.
Finally, since there is no a in the string bcd, it wouldn't find
a match.

EXERCISE

You want to find matches when the letter s occurs one or more time
in Mississippi. Write a regex that uses the + sign.

Your regex myRegex should use the + sign to match one or more s
characters.
